...
//change some style to make the English version style like the Arabic one

citydiv.style="margin-right: 10px;"
sec1.style = "direction:ltr;";
document.body.style=
"direction:rtl;";
...
for(let country in countries){
    ...
    <option>${countries[country].arabicName}</option>
    `
}
...
// add the cities of the specific chosen country
    ...
    countryBox.addEventListener("change",function(){
        if(countryBox.value == "السعودية"){
            citiesBox.innerHTML = `<option>المدن السعودية</option>`;
            for(let city in Ksacities){
                citiesBox . innerHTML += `
                <option>${Ksacities[city].arabicName}</option>
                `
            }
        }else if (countryBox.value == "مصر"){
            citiesBox.innerHTML = `<option>المدن المصرية</option>`;
            for(let city in EgyCities){
                citiesBox . innerHTML += `
                <option>${EgyCities[city].arabicName}</option>
                `}
        }
    })
            ...
            citiesBox.addEventListener("change",function(){
                ...
                for(let city of Ksacities){
                    if(city.arabicName == this.value){
                        ...
                        cityname2 = city.arabicName;
                        if(cityname == "السعودية"){
                            CountryName = "SA";
                        }
                    }
            
                }
                place.innerHTML = cityname2;
